...
//             $dao = new AdvisorDAO($pdo, $userId);
//             $service = new AdvisorService($dao);

//             try {
//                 $students = $service->getAdvisees();
...
//         // 添加顾问备注
//         $group->post('/note', function (Request $request, Response $response) use ($pdo) {
//             $userId = $_SESSION['user_id'] ?? null;
//             $data = $request->getParsedBody();

//             if (!$userId) {
//                 return $response->withStatus(401)->withHeader('Content-Type', 'application/json')
//                                 ->write(json_encode(['message' => 'Unauthorized']));
//             }
...
//             try {
//                 $service->saveNote($data['stud_id'], $data['note']);
//                 return $response->withHeader('Content-Type', 'application/json')
//                                 ->write(json_encode(['message' => 'Note saved']));
//             } catch (Exception $e) {
//                 return $response->withStatus(500)->withHeader('Content-Type', 'application/json')
...
//         // 导出学生记录为 CSV
//         $group->get('/export', function (Request $request, Response $response) use ($pdo) {
//             $userId = $_SESSION['user_id'] ?? null;
...
//             $dao = new AdvisorDAO($pdo, $userId);
//             $service = new AdvisorService($dao);
//             $data = $service->getConsultationReport();

//             $csv = fopen('php://temp', 'w+');
//             fputcsv($csv, ['Matric No', 'Name', 'GPA', 'Note']);

//             foreach ($data as $row) {
//                 fputcsv($csv, [$row['matric_no'], $row['stud_name'], $row['gpa'], $row['note']]);
//             }
